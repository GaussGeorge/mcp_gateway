...
\midrule
Core workflow & \texttt{service\_v3} & Controlled P\&S/ReAct service workflow & $C{=}\{20,50,100\}$, $F{=}\{0,0.1,0.2\}$, 5 repeats & Cascade, wasted service time & Main result \\
...
Full ablation & \texttt{full\_v1} & Mock and local vLLM layers & 84 rows & Trend checks, recovery, cascade & Mechanism \\
vLLM recovery & \texttt{vllm\_arm\_v2} & DeepSeek brain, local Qwen vLLM backend & $C{=}\{2,4\}$, 3 repeats & Recovered sessions, avoided replay & Feasibility \\
HTTP+SQLite E2E & \texttt{e2e\_smoke/perf} & External HTTP services and SQLite & 9 + 18 rows & Idempotency, DB consistency & Correctness \\
CloudLab & \texttt{cloudlab\_det\_v1} & Six nodes, Redis vs memory & Forced cross-gateway probes & State miss, recovery & Distributed correctness \\
Operator overhead & \texttt{overhead\_v1} & Same gateway/proxy path & $C{=}\{20,50,100\}$, 5 repeats & p95/p99, throughput, CPU/RSS & Deployability \\
Sensitivity & \texttt{sensitivity\_v1} & Controlled workflow grid & 126 rows & Recovery, cascade & Robustness \\
\bottomrule
\end{tabular}
}
...
\caption{Backend-dependent admission scope. The panels are not a direct performance comparison: the left panel is a single-repeat cloud backend smoke test, while the right panel is a local vLLM full run at $C{=}16$ with three repeats. They illustrate boundary behavior: provider elasticity can make no-capacity admission favorable in the cloud smoke, while constrained local vLLM exposes cascade and ABD pressure.}
