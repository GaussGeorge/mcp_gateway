...
\begin{itemize}
  ...
  \item \textbf{Empirical results.} We evaluate \plangate across controlled workflows, request-level and idempotency-only baselines, mock and vLLM backends, cloud-provider scope cases, real HTTP+SQLite services, and CloudLab distributed deployment.
\end{itemize}
...
\begin{equation}
s=\langle r_{s,0}, r_{s,1}, \ldots, r_{s,T_s-1}\rangle .
\end{equation}
...
\begin{equation}
\mathrm{Cascade}(s)=
\mathbb{1}\!\left[p_s>0 \land z_s \in
\{\mathrm{fail},\mathrm{timeout},\mathrm{rejected}\}\right].
\end{equation}
The wasted service time attributed to session $s$ is
\begin{equation}
W(s)=\mathrm{Cascade}(s)\sum_{i=0}^{p_s-1} t_{s,i},
\qquad
W(\mathcal{S})=\sum_{s\in\mathcal{S}} W(s).
\end{equation}
For durable side effects, let $O$ be the set of logical side-effect identifiers and $n_o$ be the number of committed rows or operations observed for identifier $o$. Duplicate side effects are counted as
\begin{equation}
D=\sum_{o\in O}\max(0,n_o-1).
\end{equation}
For recovery, if $c_s$ is the completed checkpoint prefix restored for session $s$, the avoided replay count is
\begin{equation}
A(s)=\mathbb{1}[\mathrm{Recovered}(s)]\,c_s .
\end{equation}
...
\[
\begin{aligned}
session\_cap\_load &= active\_sessions / session\_cap,\\
react\_step0\_load &= react\_step0\_inflight / react\_step0\_limit.
\end{aligned}
\]
...
Table~\ref{tab:adaptive_policy_defaults} reports the default thresholds used by the adaptive admission module. The current experiments use governance intensity, session-cap load, and ReAct step-0 inflight load as active signals. Cascade-rate and recoverable-error thresholds are implemented as extension hooks and remain zero in the submitted runs.

\begin{table}[t]
\centering
\caption{Default adaptive-admission state thresholds.}
\label{tab:adaptive_policy_defaults}
\footnotesize
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.42\columnwidth}>{\raggedright\arraybackslash}p{0.23\columnwidth}>{\raggedright\arraybackslash}p{0.23\columnwidth}@{}}
\toprule
\textbf{Signal / parameter} & \textbf{Green / default} & \textbf{Red / closed} \\
\midrule
Governance intensity & $<0.20$ & $\geq0.65$ \\
Session-cap load & $<0.70$ & $\geq0.90$ \\
ReAct step-0 inflight load & $<0.70$ & $\geq0.90$ \\
Cascade-rate hook & -- & $\geq0.15$ \\
Recoverable-error hook & -- & $\geq0.20$ \\
Green wait & 200 ms & -- \\
Yellow wait & 50 ms & -- \\
Red wait & -- & 0 ms \\
\bottomrule
...
Algorithm~\ref{alg:session_admission} summarizes the admission path. The key distinction is that hard rejection is evaluated before the adaptive gate, while continuation and recovery requests use session state rather than the same rule as new-session admission.

\begin{algorithm}[t]
\caption{Session-aware admission and continuation}
\label{alg:session_admission}
\footnotesize
\begin{algorithmic}[1]
\Require Request $r$, session state $\sigma$, backend pressure $\rho$, policy $\pi$
\If{\Call{ViolatesHardInvariant}{$r,\sigma$}}
  \State \Return \Call{Reject}{hard}
\EndIf
\If{\Call{IsRecoveryRequest}{$r$}}
  \State \Return \Call{ReactRecovery}{$r,\sigma$}
\EndIf
\If{\Call{IsContinuation}{$r$}}
  \If{\textbf{not} \Call{HasSessionState}{$r.session,\sigma$}}
    \State \Return \Call{Reject}{state-miss}
  \EndIf
  \State $v \gets$ \Call{ContinuationValue}{$r.session,\sigma$}
  \If{\Call{Overloaded}{$\rho$} \textbf{and not} \Call{ContinuationAdmissible}{$v,\rho$}}
    \State \Return \Call{Reject}{continuation}
  \EndIf
  \State \Return \Call{ForwardAndCheckpoint}{$r$}
\EndIf
\If{$\pi=\mathrm{no\_capacity\_step0}$}
  \State \Return \Call{AdmitNewSession}{$r$}
\ElsIf{$\pi=\mathrm{strict}$ \textbf{and} \Call{CapacityPressure}{$\rho$}}
  \State \Return \Call{Reject}{capacity-step0}
\ElsIf{$\pi=\mathrm{adaptive}$ \textbf{and} \Call{AdaptiveGateClosed}{$\rho,\sigma$}}
  \State \Return \Call{Reject}{capacity-step0}
\Else
  \State \Return \Call{AdmitNewSession}{$r$}
\EndIf
...
The design is evaluated through three admission policies. A \emph{strict} policy disables adaptive classification and rejects new sessions immediately under capacity pressure. A \emph{no-capacity-step0} policy disables capacity-driven step-0 rejection entirely. The \emph{adaptive} policy keeps hard rejection but changes capacity admission through the green/yellow/red wait policy of 200/50/0~ms for new sessions. This comparison is necessary because constrained local backends and elastic cloud providers can prefer different raw-success trade-offs.
...
Algorithm~\ref{alg:react_recovery} gives the recovery contract. It deliberately returns a resume payload rather than running the next tool call inside the gateway.

\begin{algorithm}[t]
\caption{Client-cooperative ReAct recovery}
\label{alg:react_recovery}
\footnotesize
\begin{algorithmic}[1]
\Require Resume request $r$ with session id $sid$, checkpoint store $C$
\State $c \gets$ \Call{LookupCheckpoint}{$C,sid$}
\If{$c=\varnothing$}
  \State \Return \Call{Reject}{state-miss}
\EndIf
\If{$c.status=\mathrm{active}$}
  \State \Return \Call{Reject}{live-checkpoint}
\EndIf
\If{$c.status \ne \mathrm{recoverable}$}
  \State \Return \Call{Reject}{non-recoverable}
\EndIf
\State \Call{RestoreSession}{$sid,c.completedPrefix$}
\State $payload \gets \{$recovered: true, mode: react-client-cooperative,
\Statex \hspace{\algorithmicindent} current-step: $c.nextStep$, requires-client-continuation: true,
\Statex \hspace{\algorithmicindent} avoided-replay-steps: $c.completedPrefix\}$
\State \Return $payload$
\Statex \Comment{The gateway restores state but does not execute future tools.}
\end{algorithmic}
\end{algorithm}
...
The workload layers are scoped by purpose. Controlled business workflows are the core performance setting. Mock and vLLM layers isolate backend congestion and model-backed execution \citep{kwon2023vllm}. HTTP+SQLite workflows exercise persistent side effects and idempotency. Cloud-provider smoke tests check compatibility and admission-policy scope. MCP-Bench-derived smoke and BurstGPT-shaped replay provide workflow-shape and arrival-pattern realism \citep{mcpbench2025,wang2024burstgpt}, but they are not used as headline performance rankings. Table~\ref{tab:experimental_setup_summary} summarizes the main studies.

\begin{table*}[t]
\centering
\caption{Experimental setup summary. Smoke and correctness studies are not interpreted as headline performance rankings.}
\label{tab:experimental_setup_summary}
\tiny
\renewcommand{\arraystretch}{0.82}
\resizebox{\textwidth}{!}{%
\begin{tabular}{@{}llllll@{}}
\toprule
\textbf{Study} & \textbf{Artifact} & \textbf{Backend / deployment} & \textbf{Scale / repeats} & \textbf{Primary metrics} & \textbf{Role} \\
\midrule
Core workflow & \texttt{business\_workflow\_service\_v3} & Controlled P\&S/ReAct service workflow & $C{=}\{20,50,100\}$, $F{=}\{0,0.1,0.2\}$, 5 repeats & Cascade, wasted service time & Main result \\
Request-level baseline & \texttt{request\_level\_baseline\_v1} & Same workflow harness, per-request queue admission only & $C{=}\{50,100\}$, $F{=}\{0,0.2\}$, 5 repeats & Cascade, wasted service time, request rejects & Baseline \\
Idempotency-only baseline & \texttt{idempotency\_only\_retry\_baseline\_v1} & HTTP+SQLite workflow, no resume & $C{=}\{10,20\}$, $F{=}0.1$, 3 repeats & Duplicate side effects, replayed steps & Recovery baseline \\
Full ablation & \texttt{adaptive\_step0\_react\_recovery\_full\_v1} & Mock and local vLLM layers & 84 rows & Trend checks, recovery, cascade & Mechanism \\
vLLM recovery & \texttt{vllm\_react\_recovery\_arm\_v2} & DeepSeek brain, local Qwen vLLM backend & $C{=}\{2,4\}$, 3 repeats & Recovered sessions, avoided replay & Feasibility \\
HTTP+SQLite E2E & \texttt{business\_workflow\_e2e\_smoke\_v1} / \texttt{business\_workflow\_e2e\_perf\_sanity\_v1} & External HTTP services and SQLite & 9 + 18 rows & Idempotency, DB consistency & Correctness \\
CloudLab & \texttt{cloudlab\_business\_workflow\_distributed\_deterministic\_v1} & Six nodes, Redis vs memory & Forced cross-gateway probes & State miss, recovery & Distributed correctness \\
Operator overhead & \texttt{operator\_overhead\_v1} & Same gateway/proxy path & $C{=}\{20,50,100\}$, 5 repeats & p95/p99, throughput, CPU/RSS & Deployability \\
Sensitivity & \texttt{config\_sensitivity\_v1} & Controlled workflow grid & 126 rows & Recovery, cascade & Robustness \\
\bottomrule
\end{tabular}
}
...
The artifact is organized as a versioned package rather than as a single monolithic run directory. The submitted GitHub branch contains the gateway implementation, experiment runners, generated figures, and result summaries. The canonical statistical bundle for the JSS manuscript is the versioned package \path{statistical_summary_v4}, which contains the unified statistical summary, effect-size summary, and finding table used to constrain the paper's numerical statements. A lightweight validator checks required directories, run metadata, summary row counts, duplicate-side-effect invariants, deterministic CloudLab recovery invariants, and the absence of transient runtime outputs.
...
\begin{table*}[t]
\centering
\caption{Why the two wasted-service-time comparisons should not be merged numerically. Both rows support the same waste-reduction direction, but they come from different result bundles and comparator definitions.}
\label{tab:rq1_waste_comparisons}
\footnotesize
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.22\textwidth}>{\raggedright\arraybackslash}p{0.25\textwidth}>{\raggedright\arraybackslash}p{0.17\textwidth}rrrr@{}}
\toprule
\textbf{Comparison} & \textbf{Source artifact} & \textbf{Reported quantity} & \textbf{Adaptive} & \textbf{Comparator} & \textbf{Delta} \\
\midrule
Adaptive vs no-capacity-step0 & \texttt{business\_workflow\_service\_v3} & Mean wasted service time / run at $C{=}100,F{=}0.2$ & 1{,}218 ms & 545{,}367 ms & $-544{,}149$ ms \\
Adaptive vs request-level admission & \texttt{request\_level\_baseline\_v1} & Mean wasted service time / run in the request-level baseline study at $C{=}100,F{=}0.2$ & 1{,}218 ms & 10{,}573 ms & $-9{,}355$ ms \\
\bottomrule
\end{tabular}
\end{table*}

Table~\ref{tab:rq1_waste_comparisons} makes the reporting scope explicit. The 544{,}149~ms number is the aggregate adaptive-versus-no-capacity delta from the core service-workflow artifact. The 10{,}573~ms versus 1{,}218~ms comparison comes from the request-level baseline study, which reuses the same workflow definition but reports its own comparator row. The two rows support the same direction of effect without being numerically additive.
...
\begin{table*}[t]
\centering
\caption{Idempotency-only retry versus checkpoint recovery at $C{=}20,F{=}0.1$.}
\label{tab:idempotency_vs_recovery}
\footnotesize
\resizebox{\textwidth}{!}{%
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.23\textwidth}rrrr>{\raggedright\arraybackslash}p{0.22\textwidth}@{}}
\toprule
\textbf{Variant} & \textbf{Duplicate side effects} & \textbf{Resume recovered} & \textbf{Avoided replay steps} & \textbf{Replayed completed steps} & \textbf{Interpretation} \\
\midrule
...
\caption{Client-cooperative ReAct recovery. The left panel reports HTTP service-workflow recovery; the right panel reports the targeted vLLM recovery arm. Recovery restores checkpointed progress and avoids replayed steps, but it does not execute future tools on behalf of the client.}
...
Table~\ref{tab:operator_overhead_c100} reports the $C{=}100$ resource summary. The baseline is the same gateway/proxy path rather than direct-to-backend execution. The table therefore measures incremental mechanism overhead within the gateway path, not the full cost of inserting a gateway into an otherwise direct deployment. This failure-free artifact does not trigger recovery and should not be used as recovery-effectiveness evidence.

\begin{table*}[t]
\centering
\caption{Operator overhead at $C{=}100$ in the failure-free overhead artifact.}
\label{tab:operator_overhead_c100}
\footnotesize
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.24\textwidth}rrrrrr@{}}
\toprule
\textbf{Variant} & \textbf{p95 ms} & \textbf{p99 ms} & \textbf{Throughput} & \textbf{CPU $\Delta$s} & \textbf{Avg RSS MB} & \textbf{Peak RSS MB} \\
\midrule
...
\caption{Deployability results. In the tested gateway/proxy path, p95 and throughput remain comparable, but adaptive-only can still raise p99 in a specific run. Configuration sensitivity shows that recovery strength depends on governance settings.}
...
\subsection{Agent Frameworks, Workflow Engines, and Gateways}

Agent workflow frameworks such as LangGraph, AutoGen, Semantic Kernel, and CrewAI help developers construct multi-agent applications, coordinate tool use, and represent agent state \citep{langgraphdocs2026,autogendocs2026}. They help define or execute agent logic; \plangate governs the infrastructure path after tool calls reach the gateway. Service waste, admission pressure, checkpoint lookup, and side-effect replay can arise even when the agent framework chooses tools correctly.

Workflow engines such as Temporal, Apache Airflow, Cadence, and Azure Durable Functions provide durable orchestration, scheduling, retries, and workflow state management \citep{temporaldocs2026,airflowdocs2026}. \plangate does not replace them. It is a lighter gateway mechanism for agent-generated MCP workflows where the agent or application remains responsible for future tool choice.

API gateways and service meshes such as Envoy, Kong Gateway, Istio, and Linkerd provide routing, load balancing, retries, observability, and policy enforcement for service traffic \citep{envoydocs2026,istiodocs2026}. Their policies are usually request- or route-oriented. \plangate instead makes the dependent session explicit: a late continuation step has different waste and recovery implications from a new session at step 0.

LLM gateway and router systems such as LiteLLM and Portkey focus on provider normalization, model routing, cost tracking, and OpenAI-compatible proxy behavior \citep{litellmdocs2026}. \plangate addresses a different layer: multi-tool session lifecycle, side-effect safety, and client-cooperative ReAct recovery across backend services.
...
\bibitem[Apache Airflow(2026)]{airflowdocs2026}
Apache Airflow.
\newblock Apache Airflow documentation.
\newblock \url{https://airflow.apache.org/docs/}, 2026.

\bibitem[Envoy Proxy(2026)]{envoydocs2026}
Envoy Proxy.
\newblock Envoy documentation.
\newblock \url{https://www.envoyproxy.io/docs/}, 2026.

\bibitem[Microsoft AutoGen(2026)]{autogendocs2026}
Microsoft AutoGen.
\newblock AutoGen documentation.
\newblock \url{https://microsoft.github.io/autogen/}, 2026.

\bibitem[Istio(2026)]{istiodocs2026}
Istio.
\newblock Istio documentation.
\newblock \url{https://istio.io/latest/docs/}, 2026.

\bibitem[LangChain(2026)]{langgraphdocs2026}
LangChain.
\newblock LangGraph documentation.
\newblock \url{https://langchain-ai.github.io/langgraph/}, 2026.

\bibitem[LiteLLM(2026)]{litellmdocs2026}
LiteLLM.
\newblock LiteLLM documentation.
\newblock \url{https://docs.litellm.ai/}, 2026.

\bibitem[Temporal Technologies(2026)]{temporaldocs2026}
Temporal Technologies.
\newblock Temporal documentation.
\newblock \url{https://docs.temporal.io/}, 2026.
