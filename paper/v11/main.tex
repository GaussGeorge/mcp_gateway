...
A key design choice in PlanGate's sunk-cost pricing is the use of a quadratic discount function $1/(1+K^2\alpha)$. To probe how sensitive this choice is to the discount-family shape, we compare four discount function families: \emph{quadratic} ($K^2$), \emph{linear} ($K$), \emph{exponential} ($e^{K}-1$), and \emph{logarithmic} ($\ln(1+K)$). All variants use the same $\alpha$ and session parameters.

\begin{table}[t]
\caption{Discount function diagnostic/sensitivity ablation (500 sessions, pure ReAct mode \texttt{ps\_ratio=0.0}, averaged over 5 runs). Under the current artifact summary, exponential and linear complete more sessions and achieve higher effective goodput than quadratic and logarithmic; this table is used as diagnostic evidence rather than as a proof of global optimality for any single discount family.}
\label{tab:discount}
\small
\resizebox{\columnwidth}{!}{%
\begin{tabular}{@{}lrrrrr@{}}
\toprule
\textbf{Function} & \textbf{Succ.} & \textbf{Casc.} & \textbf{Eff.GP/s} & \textbf{P95} & \textbf{JFI$_s$} \\
\midrule
Quadratic ($K^2$)              & 3.2            & 26.8          & 0.29          & \textbf{5.4}  & 1.000 \\
Linear ($K$)                   & 4.6            & 25.4          & 0.39          & 13.9          & 1.000 \\
\textbf{Exponential} ($e^K$)  & \textbf{5.0}  & \textbf{25.0} & \textbf{0.42} & 14.5          & 1.000 \\
Logarithmic ($\ln(1{+}K)$)    & 3.4            & 26.6          & 0.30          & 5.5           & 1.000 \\
\bottomrule
\end{tabular}}
\end{table}

Table~\ref{tab:discount} should be read narrowly as current diagnostic evidence from the artifact summary. Under this CSV, exponential achieves the highest mean success (5.0), the lowest mean cascade count (25.0), and the highest effective goodput (0.42~GP/s), with linear next at 4.6 successes, 25.4 cascades, and 0.39~GP/s. Quadratic and logarithmic are weaker in the current summary (3.2/3.4 successes and 26.8/26.6 cascades, respectively). At the same time, the cascade counts remain clustered in the 25--27 range and the fairness metric no longer separates the policies: JFI$_s$ is approximately 1.0 for all four variants. The appropriate interpretation is therefore sensitivity/diagnostic ranking under this workload, not a broad empirical claim that one family is globally best.

We nevertheless keep the quadratic discount ($K^2$) as the default practical operating point for mechanism-design reasons rather than because Table~\ref{tab:discount} makes it the empirical winner. The quadratic form is simple to compute on the admission hot path, grows monotonically and predictably with session depth, and is easier to reason about as a conservative continuation policy than exponential growth, which can become more permissive for deeper sessions. In that sense, the current Exp8 ablation is best understood as diagnostic/sensitivity evidence about how different discount families behave in one summarized workload, not as proof that quadratic is globally optimal. 
...
Second, there is no single-policy dominance trend in these smoke/boundary runs:
...
used as blanket-ranking evidence.
...
The new multi-intensity profile sweep extends this boundary reading from a single saturated point to $C\in\{8,12,16,20\}$ using the same submitted 5-gateway subset. Figure~\ref{fig:selfhosted_vllm_profile_sweep} shows that PlanGate (tuned) is \emph{not} uniformly best at lower contention: at $C{=}8$, its success rate is $77.9\%$, below PP ($90.0\%$) and Rajomon ($91.3\%$). Under tighter local-vLLM saturation, however, the tuned policy moves into a stronger completion band: at $C{=}20$, PlanGate (tuned) reaches $28.3\%$ success versus NG's $16.3\%$, Static's $15.8\%$, PP's $19.2\%$, and Rajomon's $23.3\%$, while also lowering ABD relative to NG/Static/PP. The statistical summary artifact records this as a $+12.0$ percentage-point success delta against NG at $C{=}20$, but also as a $-13.3$ percentage-point delta against Rajomon at $C{=}8$. The correct interpretation is therefore boundary characterization: once backend-local scarcity is explicit, session-aware tuning can recover useful completion relative to several baselines, but low-contention points do not establish one policy as best everywhere.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_selfhosted_vllm_profile_sweep}
\caption{Self-hosted vLLM multi-intensity boundary characterization from \texttt{artifact\_results/selfhosted\_vllm\_profile\_sweep\_v1}. PlanGate (tuned) is not the best gateway at $C{=}8$, but its success/ABD trade-off improves relative to several baselines once the local vLLM backend enters the harsher $C{=}16/20$ regime.}
\label{fig:selfhosted_vllm_profile_sweep}
\end{figure}
...
The newer CloudLab artifact repeats the same semantic question with the post-P3 checkpoint-store path enabled and random routing across gateways, but now with an explicit Redis-vs-memory control. Table~\ref{tab:cloudlab_redis_memory} aggregates the lightweight CloudLab summary: across three Redis-backed runs, random routing produces 21{,}877 cross-node sessions with \texttt{state\_miss=0}; the matched in-memory control records 12{,}398 cross-node sessions and 9{,}813 state misses. The diagnostic-control adversarial rows also separate cleanly: Redis rejects all 54 adversarial rows, while the memory control does so in only 9/54. This narrows the deployment claim to shared-state lookup correctness under cross-gateway continuation. It does not claim that Redis by itself provides a complete replicated control plane.

\begin{table}[t]
\centering
\caption{CloudLab random-routing Redis-vs-memory shared-state check from \texttt{artifact\_results/cloudlab\_random\_redis\_memory\_v1}. The Redis arm is the correctness arm; the in-memory arm is the diagnostic control.}
\label{tab:cloudlab_redis_memory}
\small
\resizebox{\columnwidth}{!}{%
\begin{tabular}{@{}lrrrr@{}}
\toprule
\textbf{Store} & \textbf{Runs} & \textbf{Cross-node} & \textbf{StateMiss} & \textbf{Reject-all Rate} \\
\midrule
Redis  & $3$ & $21{,}877$ & $0$     & $1.000$ \\
Memory & $3$ & $12{,}398$ & $9{,}813$ & $0.167$ \\
\bottomrule
\end{tabular}}
\end{table}
...
The new P3 failure/amendment grid extends that claim across six $(r,a)$ cells with $r\in\{0.1,0.2,0.3\}$ and amendment rate $a\in\{0.1,0.2\}$. Its signal is again mechanism consistency rather than a blanket throughput ranking. In all six cells, disabling recovery drives \texttt{recovery\_success} to zero and raises cascade failure relative to Full; in all six cells, disabling amendment drives \texttt{amendment\_success} to zero. Success-rate ordering is mostly, but not universally, favorable to Full: for example, at $(r{=}0.1,a{=}0.1)$, \texttt{wo\_commitment} is slightly ahead on mean success ($187.3$ vs.\ $184.7$), whereas at $(r{=}0.3,a{=}0.2)$ Full leads \texttt{wo\_recovery} by $59.7$ successful sessions and reduces cascade by the same magnitude. We therefore use the grid as boundary-sensitive mechanism evidence: most settings preserve the intended signal, while low-failure cells mark the edge of where Full is not strictly best on every aggregate metric.
...
The $K^2$ discount is one instantiation of session commitment, not the core contribution. The model in \S\ref{sec:discount-rationale} motivates it by reducing expected waste from $O(N^2)$ to $O(\ln N)$ under simplified assumptions; the discount function ablation (\S\ref{sec:discountablation}) should be read as sensitivity evidence rather than as proof that any one family is best in every workload. In the current artifact summary, exponential and linear achieve slightly higher success and effective goodput than quadratic and logarithmic, while cascade counts remain close and JFI no longer separates the variants. We nevertheless choose $K^2$ over $e^K$ as the default because it is cheap to compute and grows less permissive for very deep sessions. Future work could adapt $\alpha$ online: increase it when ABD exceeds a target, and decrease it when step-0 rejections exceed a utilization target. Our $\alpha$ sweep suggests a wide operating envelope rather than a brittle hyperparameter. The load-modulation coefficient $\beta$ is similarly not a brittle constant: the Supplementary Material, Section~3 sweep over $\beta\in\{0,0.5,1,2,3\}$ shows identical completion counts across all settings, with only small cascade/fairness tradeoffs.
...
The main experiments evaluate a 10-tool ecosystem spanning light and heavy costs. More tools would extend the weight table, but production deployments may require finer-grained weights, tool-specific quotas, and multi-node shared state for reservations, prices, checkpoints, and reputation. Section~\ref{sec:multigateway} validates a first step beyond process-local state: under random CloudLab routing, Redis-backed shared state records zero state misses and zero duplicate admissions across 21,877 cross-node sessions, while the memory-local diagnostic control exposes 9,813 state misses over 12,398 cross-node sessions. This is shared-state correctness evidence rather than a complete distributed control plane. Shared ReAct progress, distributed reputation, dynamic price-state synchronization, node-failure recovery, and production-grade store replication remain future work. Mock workloads use \texttt{time.sleep} to isolate admission behavior; real-LLM steady, bursty, self-hosted, GLM, and DeepSeek smoke experiments mitigate this realism threat but do not replace production traces.

\textbf{Real-provider interpretation.}
Provider-backed experiments are intentionally interpreted conservatively. Commercial APIs add hidden queuing, elastic rate-limit headroom, stochastic generation latency, and provider-specific tool-call response behavior. The GLM and DeepSeek evidence is therefore used as smoke/support and boundary characterization rather than as headline ranking evidence. The core causal claims remain grounded in controlled workloads where backend capacity and failure injection are known.
