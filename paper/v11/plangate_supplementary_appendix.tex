...
Table~\ref{tab:selfhosted_profile_appendix} reproduces the complete aggregate CSV from the self-hosted vLLM profile-sweep artifact. The main-paper interpretation is intentionally bounded: the $C{=}8$ row set preserves the low-contention region where PlanGate (tuned) is not the top success-rate gateway, while the $C{=}16/20$ rows show the harsher saturation regime where its success/ABD trade-off improves relative to several baselines.

\noindent\textit{Source bundle:} \path{artifact_results/selfhosted_vllm_profile_sweep_v1/}

\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{fig_selfhosted_vllm_profile_sweep}
\caption{Full self-hosted vLLM profile sweep generated from the profile-sweep aggregate CSV.}
\label{fig:selfhosted_profile_appendix}
\end{figure}

\begin{table*}[h]
\centering
\caption{Complete self-hosted vLLM profile-sweep aggregate table. P95 is reported in seconds.}
...
Table~\ref{tab:p3_grid_appendix} reproduces the complete aggregate CSV from the P3 failure/amendment grid artifact. Figure~\ref{fig:p3_grid_appendix} visualizes the same boundary: the dominant qualitative signal is stable mechanism activation, not that Full wins every cell on every aggregate metric.

\noindent\textit{Source bundle:} \path{artifact_results/p3_failure_amendment_grid_v1/}

\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{fig_p3_failure_amendment_grid}
\caption{P3 failure/amendment grid generated from the aggregate CSV.}
\label{fig:p3_grid_appendix}
\end{figure}

\begin{table*}[h]
\centering
\caption{Complete P3 failure/amendment grid aggregate.}
...
Table~\ref{tab:cloudlab_random_redis_memory_appendix} reproduces the per-run summary CSV from the CloudLab Redis-vs-memory artifact. The Redis arm is the correctness arm and the in-memory arm is the diagnostic control. The table should be read narrowly as shared-state lookup evidence under random routing.

\noindent\textit{Source bundle:} \path{artifact_results/cloudlab_random_redis_memory_v1/}

\begin{table*}[h]
\centering
\caption{Complete CloudLab Redis-vs-memory per-run summary.}
...
The complete statistical-summary and effect-size CSV files are preserved as machine-readable appendix listings colocated with this appendix source. They are not typeset row-by-row in the PDF because the files are wide and primarily intended for reproducibility checks. Table~\ref{tab:statistical_summary_files} records the shipped files and their scope. These listings should be read with the same evidence hierarchy as the main text: CloudLab rows are state-consistency evidence, provider rows remain boundary evidence if added later, and diagnostics such as Exp8 remain explicitly labeled diagnostic.

\begin{table*}[h]
\caption{Machine-readable statistical appendix files shipped next to this appendix source.}
\label{tab:statistical_summary_files}
\small
\begin{tabular}{@{}lllp{0.48\textwidth}@{}}
\toprule
File & Rows & Source artifact & Role \\
\midrule
\texttt{statistical\_summary.csv} & 405 & \texttt{statistical\_summary\_v1} & Bootstrap/statistical summaries over the existing evidence CSVs, including core mock results, P3 ablations and grid, vLLM sweep, CloudLab shared-state check, and throughput/latency summaries. \\
\texttt{effect\_size\_summary.csv} & 202 & \texttt{statistical\_summary\_v1} & Pairwise deltas and confidence intervals for the core comparisons used in the main-paper claims and appendix interpretation. \\
\bottomrule
\end{tabular}
\end{table*}
