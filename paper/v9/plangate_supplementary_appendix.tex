...
The self-hosted vLLM smoke run is retained as a diagnostic note only and is excluded from the paper's claims, because it did not establish a stable proxy-baseline comparison.
...
In PlanGate's P\&S mode, sessions declare a DAG at admission; budget reservation prevents price-induced rejection within the declared plan scope under the stated assumptions. ReAct sessions cannot declare future steps, so risk exposure $R(K) = \sum_{i=1}^{K} c_i$ grows with each step and the discount function directly controls the tradeoff. The convex $d(K)=1/(1+K^2\alpha)$ captures the acceleration: at $K{=}3$ with $\alpha{=}0.5$, the effective price drops to 18\% of base. A linear discount $d(K)=1/(1+K\alpha)$ achieves only $O(N)$ waste; a constant discount ignores sunk cost entirely. The discount function ablation in the main paper empirically confirms: $K^2$ outperforms linear (21.0 cascades) and logarithmic (31.2) in cascade prevention.
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{figures/exp10_adversarial.pdf}
...
% ================================================================
\section{Self-Hosted vLLM: High-Contention Regime ($C{=}20$)}
\label{sec:selfhosted_c20}
\label{app:vllm-high}
% ================================================================

The self-hosted vLLM high-contention experiment ($C{=}20$, 100 agents, 8 GPU workers, $N{=}3$) is reported in full in the main paper Section~\ref{sec:selfhosted} and Table~\ref{tab:selfhosted}.
