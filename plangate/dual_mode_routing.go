...
// │         α_eff = α · (1 + β · (1 − I(t)))                             │
// │         β=1.0 时等价旧形式 α·(2−I(t))（默认值）                      │
// │                                                                       │
// │ 二次折扣核心: K² 使折扣随步数激进递增                                 │
// │   K=3, α=0.4, β=1, I=1.0 → α_eff=0.4, δ=1/(1+3.6)=0.22 (78% 折扣)  │
// │   强度调制: I=0.5, β=1 → α_eff=1.5α → 低负载时更强保护               │
// │                                                                       │
...
func (s *MCPDPServer) handleReActSunkCostStep(
	...
	if s.protectCommittedSessions {
		...
		if err != nil {
			...
			// PlanGate-R Phase 4A: 标记 ReAct committed 会话的 checkpoint 为 recoverable。
			s.markCheckpointRecoverable(ctx, rState.SessionID, classifyTransportError(err))
			return mcpgov.NewErrorResponse(req.ID, mcpgov.CodeInternalError, err.Error(), nil)
		}
	...
	if intensity < 0.01 {
		...
		//   Step B: α_eff = α · (1 + β · (1 − I(t)))
		//     beta=1.0 时等价旧公式 α·(2−I(t))；beta=0 时 α_eff=α（无强度调制）
		//     I=1.0 → α_eff=α；I=0.5, beta=1 → α_eff=1.5α（折扣增强 50%）
		//     beta 越大，低负载时折扣越激进，保护已走多步的会话
		effectiveAlpha := s.sunkCostAlpha * (1.0 + s.sunkCostBeta*(1.0-intensity))
	...
	if err != nil {
		...
		// PlanGate-R Phase 4A: 标记 ReAct 会话的 checkpoint 为 recoverable。
		s.markCheckpointRecoverable(ctx, rState.SessionID, classifyTransportError(err))
		return mcpgov.NewErrorResponse(req.ID, mcpgov.CodeInternalError, err.Error(), nil)
	}
...
func (s *MCPDPServer) executeStepDirectWithShared(
	...
	if err != nil {
		...
		// PlanGate-R Phase 4A: 尝试标记 P&S 活跃 checkpoint 为 recoverable。
		// 动作：如果 checkpoint store 里已有该 session 的 ACTIVE_CHECKPOINT，
		// 剩正将其升级为 CHECKPOINTED，不改变客户端看到的错误。
	...
	// 同时用 nextPSStepState 判断是否完成 DAG 所有步骤。
	//
	// 语义一致性保证：
	//   nextStep, complete := nextPSStepState(preAdvanceStep, totalSteps)
	//   nextStep  = preAdvanceStep + 1  = 下一个待执行步骤 index
	//   complete  = nextStep >= total   = 所有步骤已执行完毕
	//   checkpoint.CurrentStep = nextStep（= "下一个待执行 step index"，语义明确）
	if hasRes {
