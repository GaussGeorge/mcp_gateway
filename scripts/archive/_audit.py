...
paper_c10 = {
    # v7 corrected: C=10 is conservative-admission PENALTY boundary
    'ng':            {'succ%': 94.0, 'succ_std': 1.3, 'abd': 5.5, 'abd_std': 1.0, 'gps': 0.40, 'gps_std': 0.01, 'p95': 80.5},
    'rajomon':       {'succ%': 94.0, 'succ_std': 1.0, 'abd': 5.0, 'abd_std': 1.6, 'gps': 0.40, 'gps_std': 0.01, 'p95': 73.1},
    'pp':            {'succ%': 92.1, 'succ_std': 4.2, 'abd': 7.2, 'abd_std': 3.7, 'gps': 0.39, 'gps_std': 0.03, 'p95': 75.7},
    'plangate_real': {'succ%': 88.4, 'succ_std': 2.4, 'abd': 10.1, 'abd_std': 2.5, 'gps': 0.33, 'gps_std': 0.02, 'p95': 87.4},
}
...
print()
print("="*80)
print("TABLE 10: tab:selfhosted (exp_selfhosted_vllm_C20_W8) [C=20, N=3]")
print("="*80)
# NOTE: C=10 runs (exp_selfhosted_vllm_C10_W8) are DEPRECATED — those runs used
# Brain=qwen (env override), not GLM-4-Flash, causing 100% success (no contention).
# The canonical paper Table 8 uses C=20 data.
...
paper_c20 = {
    'ng':            {'succ%': 12.3, 'succ_std': 5.4, 'abd': 78.4, 'casc': 98, 'casc_std': 21, 'p95': 101},
    'plangate_real': {'succ%': 8.7,  'succ_std': 3.1, 'abd': 83.6, 'casc': 85, 'casc_std': 9,  'p95': 117},
}
...
for gw in ['ng', 'plangate_real']:
    ...
    rej0 = [float(r['all_rejected']) for r in runs]
    p95  = [float(r['p95_ms']) / 1000.0 for r in runs]
    sm, ss = mean_std(sr)
    cm, cs = mean_std(casc)
    rm     = statistics.mean(rej0)
    p95m   = statistics.mean(p95)
    p = paper_c20[gw]
    issues = []
    if abs(sm - p['succ%']) > 0.15: issues.append(f"Succ%: CSV={sm:.1f} paper={p['succ%']}")
    ...
    status = "OK" if not issues else "ISSUES"
    print(f"  {gw:20s}: Succ%={sm:.1f}+-{ss:.1f} Casc={cm:.0f}+-{cs:.0f} Rej0={rm:.1f} P95={p95m:.0f}s  [{status}]")
    for iss in issues:
        print(f"    ** {iss}")

print("  Paper claims: NG success=12.3%, cascade=98+-21, P95=101s")
print("  Paper claims: PG success=8.7%,  cascade=85+-9,  P95=117s")

# =====================================================================
print()
print("="*80)
print("TABLE 12: tab:bursty_reallm (exp_bursty_C20_B30, N=7)")
print("="*80)
# Already verified in n9_stats_output.txt, just confirm key numbers
# Load the bursty_summary.csv which only has partial data, but _compute_bursty_stats showed the full picture
print("  (Verified from _compute_bursty_stats.py output; first-valid-row-per-run, neutral rows excluded)")
print("  NG:  PARTIAL=94+-14, Cascade=174+-38, Rej0=86+-15  [N=7, v7 corrected]")
print("  PG:  PARTIAL=76+-12, Cascade=143+-25, Rej0=108+-12  [N=7, t(12)=2.73, p<0.02]")
print("  All match paper v7 claims.")
