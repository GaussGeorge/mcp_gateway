...
Includes N=7 bursty data and all other experiment data.
...
def collect_bursty_n7():
    """Collect N=7 bursty data from per-run CSVs (neutral harness rows excluded)."""
        ...
        for csv_path in run_dirs:
            with open(csv_path, newline="", encoding="utf-8-sig") as f:
                ...
                for row in reader:
                    ...
                    # Skip harness-injected neutral rows (backend_llm_tokens=0)
                    if int(row.get(backend_tok, 0)) == 0:
                        continue
                    data[gw].append({
                    ...
                    break  # only first valid row per run → N=7 data points
    return data
...
def fig_mock_cascade():
    """Exp1 cascade failure bar chart — tab:exp1 data (N=5 runs, 500 sessions)."""
    gws      = ["NG", "SRL", "SBAC", "PlanGate"]
    cascade  = [115.6, 106.6, 33.6, 0.0]
...
def fig_exp4_ablation():
    """Exp4 ablation study: 3 variants — tab:ablation data (N=5 runs, 500 sessions)."""
    variants = ["PlanGate\nFull", "w/o\nBudgetLock", "w/o\nSessionCap"]
    success  = [81.0, 19.0, 81.6]
    cascade  = [1.4, 11.0, 1.0]
    gps      = [59.3, 12.1, 59.1]
...
def fig_token_efficiency():
    """Token cost per successful task — read from exp_real3 summary CSVs (N=5 per gateway)."""
    provider_config = [
        ("GLM-4-Flash",  os.path.join(RES, "exp_real3_glm",      "summary_all.csv")),
        ("DeepSeek-V3",  os.path.join(RES, "exp_real3_deepseek",  "summary_all.csv")),
    ]
    gw_order  = ["ng", "srl", "mcpdp-real", "mcpdp-real-no-sessioncap"]
    gw_labels = ["NG", "SRL", "PlanGate", "PG w/o SC"]
    gw_colors = [C_NG, C_SRL, C_PG, C_PGNR]
    gw_hatches = ["xxx", "\\\\\\\\", "///", "..."]

    fig, axes = plt.subplots(1, 2, figsize=(COL_W * 2.0, 2.2))

    for p_idx, (prov, csv_path) in enumerate(provider_config):
        ax = axes[p_idx]
        if not os.path.isfile(csv_path):
            ax.set_title(f"{prov}\n(data missing)")
            continue

        # Compute tokens/success per run for each gateway
        gw_vals = {gw: [] for gw in gw_order}
        with open(csv_path, newline="") as f:
            reader = csv.DictReader(f)
            for row in reader:
                gw = row.get("gateway", "")
                if gw in gw_vals:
                    tok  = float(row.get("agent_llm_tokens", 0) or 0)
                    succ = float(row.get("success", 0) or 0)
                    if succ > 0:
                        gw_vals[gw].append(tok / succ)

        x = np.arange(len(gw_order))
        max_top = 0.0
        for i, (gw, label, color, hatch) in enumerate(zip(gw_order, gw_labels, gw_colors, gw_hatches)):
            vals = gw_vals[gw]
            if not vals:
                continue
            mean_v = statistics.mean(vals)
            std_v  = statistics.stdev(vals) if len(vals) > 1 else 0.0
            bar = ax.bar(i, mean_v, yerr=std_v, width=0.55, color=color,
                         edgecolor="black", linewidth=0.5, capsize=3,
                         error_kw={"linewidth": 0.8})
            bar[0].set_hatch(hatch)
            ax.text(i, mean_v + std_v + 80, f"{mean_v:,.0f}",
                    ha="center", va="bottom", fontsize=6.5, fontweight="bold")
            max_top = max(max_top, mean_v + std_v)
    ...
    fig.suptitle("Token Efficiency per Successful Task (lower is better)", fontsize=10, y=1.02)
    fig.tight_layout()
...
# ═════════════════════════════════════════
# NEW: Bursty Real-LLM N=7 figures
# ═════════════════════════════════════════
def fig_bursty_reallm():
    """Generate bursty real-LLM comparison figure with N=7 data."""
    data = collect_bursty_n7()
    ...
    ax.set_ylabel("Count")
    ax.set_title(f"(a) PARTIAL\n(-{reduction_p:.1f}%, p<0.02)", fontsize=8)
...
# ═════════════════════════════════════════
def fig_selfhosted():
    """Self-hosted vLLM comparison (C=20, N=3)."""
    ...
    ax.set_title("(b) Rej₀ (n.s.)", fontsize=8)
    ...
    fig.suptitle("Self-hosted vLLM (Qwen-3.5-4B, C=20, N=3)", fontsize=9, y=1.02)
    fig.tight_layout()
    savefig(fig, "selfhosted_vllm_c20")
...
def fig_tput_latency():
    """
    Exp13_TputLatency: 3-panel throughput–latency figure.
    Panel (a): P95 latency (s) vs effective goodput (GP/s) — scatter+line per gateway
    Panel (b): Effective GP/s vs concurrency (offered load)
    Panel (c): ABD% + cascade waste% vs concurrency

    Reads: results/exp_tput_latency/tput_latency_agg.csv
    """
    agg_csv = os.path.join(RES, "exp_tput_latency", "tput_latency_agg.csv")
    if not os.path.isfile(agg_csv):
        print(f"  SKIP fig_tput_latency — {agg_csv} not found (run run_tput_latency_sweep.py first)")
        return
    ...
    for r in rows:
        ...
        try:
            ...
            d["gps_ci"].append(float(r["gps_ci95"]))
            d["p95"].append(float(r["p95_mean"]))
            d["p95_ci"].append(float(r["p95_ci95"]))
            d["abd"].append(float(r["abd_pct_mean"]))
            d["casc"].append(float(r["casc_pct_mean"]))
        except (KeyError, ValueError):
            continue

    # Sort each gateway's lists by concurrency
    ...
    GW_STYLE = {
        "ng":            {"label": "No-Gate",  "color": C_NG,   "marker": "o", "ls": "-"},
        "srl":           {"label": "SRL",      "color": C_SRL,  "marker": "^", "ls": "--"},
        "sbac":          {"label": "SBAC",     "color": C_SBAC, "marker": "s", "ls": "-."},
        "plangate_full": {"label": "PlanGate", "color": C_PG,   "marker": "D", "ls": "-"},
    }

    fig, axes = plt.subplots(1, 3, figsize=(DBL_W, 2.3))
    ax_scatter, ax_gps, ax_abd = axes
    ...
    for gw, d in gw_data.items():
        ...
        ax_scatter.errorbar(
            d["gps"], d["p95"],
            xerr=d["gps_ci"], yerr=d["p95_ci"],
            color=st["color"], marker=st["marker"],
            linewidth=1.1, markersize=5, capsize=2,
            label=st["label"], linestyle=st["ls"],
        )
        # annotate concurrency level on PlanGate curve
        if gw == "plangate_full":
            for x, y, c in zip(d["gps"], d["p95"], d["conc"]):
                ax_scatter.annotate(
                    f"C={c}", (x, y),
                    textcoords="offset points", xytext=(4, 3),
                    fontsize=5.5, color=C_PG, alpha=0.85,
                )

    ax_scatter.set_xlabel("Effective Goodput (sess/s)", fontsize=9)
    ax_scatter.set_ylabel("P95 Latency (s)", fontsize=9)
    ax_scatter.set_title("(a) Latency–Goodput Trade-off", fontsize=9)
    ax_scatter.legend(fontsize=7, ncol=1, handlelength=1.5)

    # ---- panel (b): GP/s vs concurrency ----
    ...
    ax_gps.legend(fontsize=7, handlelength=1.5)

    # ---- panel (c): Admitted-session failure rate vs concurrency ----
    gw_list = ["ng", "srl", "sbac", "plangate_full"]
    # Only include gateways that have data
    gw_list = [gw for gw in gw_list if gw_data[gw]["conc"]]

    for gw in gw_list:
        d = gw_data[gw]
        st = GW_STYLE[gw]
        ax_abd.plot(d["conc"], d["casc"], color=st["color"], marker=st["marker"],
                    linewidth=1.1, markersize=5, linestyle=st["ls"],
                    label=f"{st['label']}")

    ax_abd.set_xlabel("Concurrency (offered load)", fontsize=9)
    ax_abd.set_ylabel("Admitted Sessions (%)", fontsize=9)
    ax_abd.set_title("(c) Admitted Failure Rate", fontsize=9)
    ax_abd.set_xticks(conc_vals)
    ax_abd.legend(fontsize=7, ncol=1, handlelength=1.2)

    fig.tight_layout()
    savefig(fig, "tput_latency_sweep")
...
def main():
    ...
    print("\n[10/10] Exp13 throughput-latency sweep")
    fig_tput_latency()

    print("\n" + "=" * 50)
    print(f"9 data figures regenerated to {FIG_DIR}")
    print("=" * 50)
