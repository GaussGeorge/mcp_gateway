...
# ── 优先加载 .env（模块级 os.environ.get 在此之后才能读到 .env 变量）──
def _load_dotenv():
...
# ══════════════════════════════════════════════════
# Agent brain: 从环境变量读取 (商业 API 用于 agent 大脑)
# 优先顺序: AGENT_LLM_BASE_URL > AGENT_LLM_BASE > LLM_API_BASE > default GLM
# ══════════════════════════════════════════════════
AGENT_LLM_BASE = os.environ.get("AGENT_LLM_BASE_URL",
                  os.environ.get("AGENT_LLM_BASE",
                  os.environ.get("LLM_API_BASE", "https://open.bigmodel.cn/api/paas/v4")))
AGENT_LLM_KEY = os.environ.get("AGENT_LLM_KEY", os.environ.get("LLM_API_KEY", ""))
AGENT_LLM_MODEL = os.environ.get("AGENT_LLM_MODEL", os.environ.get("LLM_MODEL", "glm-4-flash"))

# ══════════════════════════════════════════════════
# Experiment Parameters (bursty, matching bursty real-LLM)
# ══════════════════════════════════════════════════
AGENTS = 100        # 增大 (50→100) 使 vLLM bottleneck 更明显
CONCURRENCY = 20    # 增大 (10→20) 使 vLLM 资源争抢更激烈
MAX_STEPS = 10
BUDGET = 800
BURST_SIZE = 15
BURST_GAP = 6.0
...
BACKEND_PROC = None
_OVERRIDE_OUT_DIR = None  # set by --out-dir arg


def get_results_dir():
    if _OVERRIDE_OUT_DIR:
        return _OVERRIDE_OUT_DIR
...
def run_experiment(repeats: int, dry_run: bool = False, resume: bool = False):
    ...
    for gw in GATEWAYS:
        for run_idx in range(1, repeats + 1):
            ...
            if resume and not dry_run:
                agents_csv = os.path.join(run_dir, "steps_agents.csv")
                if os.path.exists(agents_csv):
                    try:
                        row_count = sum(1 for _ in open(agents_csv, encoding="utf-8-sig")) - 1
                    except Exception:
                        row_count = 0
                    if row_count > 0:
                        print(f"  [resume] 跳过 {gw.name}/run{run_idx} ({row_count} agents 已存在)")
                        continue
...
def main():
    ...
    if args.out_dir is not None:
        _OVERRIDE_OUT_DIR = os.path.abspath(args.out_dir)
    ...
    global GATEWAYS
    if args.gateways:
        GATEWAYS = [gw for gw in GATEWAYS if gw.name in args.gateways]
        if not GATEWAYS:
            print(f"错误: 未找到指定网关 {args.gateways}")
            sys.exit(1)

    print(f"  Agents: {AGENTS}, Concurrency: {CONCURRENCY}")
    print(f"  Results: {get_results_dir()}")

    build_gateway()
    start_backend()
    try:
        run_experiment(args.repeats, dry_run=args.dry_run, resume=args.resume)
    finally:
        stop_backend()
